# -*- coding: utf-8 -*-
"""
Created on Thu Apr 14 08:40:18 2022
...
if __name__ == '__main__':
    ...
    B_arr   = np.linspace(0.,2.,21)**2 *Om_L # Bfield array
    Om_arr  = np.logspace(np.log10(0.25),np.log10(7),9) *Gamma # Rabi frequencies
    ...
    for j,Om in enumerate(Om_arr):
        ...
        plt.plot(B_arr/Om_L, 2*system.results[0]['Ne'][:,j],
                 label=r'${:.2f}$'.format(Om/Gamma))
    plt.legend(title='$\Omega$ in $\Gamma$')
    plt.xlabel(r'Larmor frequency $\omega_L$ in $\Gamma$')
    plt.ylabel('Scattering rate $R_{sc}$ in $\Gamma/2$')
